...
import { createQuotation } from '../../api/qot'
...
export default function TeklifYeniPage() {
  const navigate = useNavigate()
  const toast = useToast()
  const [searchParams] = useSearchParams()
  const mode = searchParams.get('type') === 'alis' ? 'alis' : 'satis'
  const documentType = mode === 'alis' ? 'PURCHASE' : 'SALES'
  const listPath = '/teklif'
  const title = mode === 'alis' ? 'Yeni Alış Teklifi' : 'Yeni Satış Teklifi'
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      toast.success('Kayıt oluşturuldu', 'Teklif listeye eklendi.')
      navigate(listPath)
    } catch (err: unknown) {
      const message = apiErrorMessage(err, 'Teklif kaydedilemedi.')
      setError(message)
  ...
  return (
    <div className="app-page-content">
      <div className="page-header d-flex flex-wrap justify-content-between align-items-start gap-3 mb-4">
        ...
        <div className="d-flex align-items-center flex-wrap gap-2">
          ...
          <button
            ...
            data-bs-target="#modalCariSecTeklif"
          >
              ...
              <div className="col-md-4">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setValidUntil(addDaysIso(e.target.value, 30))
                  }}
                  required
                />
              </div>
      ...
      <CariSecModal
        modalId="modalCariSecTeklif"
        description={
          mode === 'alis'
            ? 'Teklife bağlamak istediğiniz tedarikçi cariyi listeden seçin.'
            : 'Teklife bağlamak istediğiniz müşteri cariyi listeden seçin.'
        }
        cariler={cariler}
        loading={loading}
