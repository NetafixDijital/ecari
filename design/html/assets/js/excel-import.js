(function () {
  ...
  function mapCariRows(rawRows) {
    return rawRows.map(function (raw) {
      ...
      var mapped = {
        code: pick(r, ['code', 'cari_kod', 'kod', 'cari kodu']),
        person_type: pick(r, ['person_type', 'cari_tipi', 'tip', 'musteri_tipi', 'musteri tipi']),
        title: pick(r, ['title', 'cari_unvan', 'unvan', 'ad_soyad', 'ad soyad', 'firma']),
        tax_number: pick(r, ['tax_number', 'vkn', 'vergi_no', 'vergi no']),
        identity_number: pick(r, ['identity_number', 'tckn', 'tc_kimlik']),
        tax_office: pick(r, ['tax_office', 'vergi_dairesi', 'vergi dairesi']),
        address_line: pick(r, ['address_line', 'adres']),
        email: pick(r, ['email', 'eposta', 'e_posta']),
        phone: pick(r, ['phone', 'telefon', 'tel']),
        balance: pick(r, ['balance', 'bakiye']) || '0.00',
        balance_side: pick(r, ['balance_side', 'bakiye_tip', 'bakiye tip']) || '',
        is_active: pick(r, ['is_active', 'durum']) || 'aktif'
      };
      var legacyTax = pick(r, ['vkn_tckn']);
      if (legacyTax && !mapped.tax_number && !mapped.identity_number) {
        if (legacyTax.length === 11) mapped.identity_number = legacyTax;
        else mapped.tax_number = legacyTax;
      }
      if (window.CariDbSim) return window.CariDbSim.fromExcelRow(mapped);
      return mapped;
    }).filter(function (row) {
  ...
  function mapStokRows(rawRows) {
    return rawRows.map(function (raw) {
      var r = normalizeRow(raw);
      var mapped = {
        code: pick(r, ['code', 'stok_kod', 'kod', 'stok kodu', 'barkod']),
        name: pick(r, ['name', 'urun', 'urun_adi', 'urun adi', 'stok_adi', 'stok adi', 'ad']),
        base_unit_name: pick(r, ['base_unit_name', 'birim']) || 'Adet',
        quantity_on_hand: pick(r, ['quantity_on_hand', 'miktar', 'stok', 'adet']) || '0',
        purchase_gross: pick(r, ['purchase_gross', 'alis', 'alis_fiyati', 'alis fiyati', 'alış']) || '0.00',
        sales_gross: pick(r, ['sales_gross', 'satis', 'satis_fiyati', 'satis fiyati', 'satış']) || '0.00',
        durum: pick(r, ['durum', 'stock_status']) || 'aktif'
      };
      if (window.EcariDbSim) return window.EcariDbSim.fromStokExcelRow(mapped);
      return mapped;
    }).filter(function (row) {
      return (row.code || row.stok_kod) && (row.name || row.urun);
    });
  }
