(function () {
  ...
  function updateBadge() {
    ...
    var headerBadge = document.querySelector('[data-notification-header-badge]');
    if (headerBadge) headerBadge.textContent = count;
  }

  function isNexlinkList() {
    var list = document.getElementById('ecari-notifications');
    return list && list.classList.contains('list-group-unlined');
  }
  ...
  function renderNexlinkItem(item) {
    var bg = AVATAR_BG[item.type] || 'bg-secondary';
    var unreadCls = item.unread ? ' nl-notify-unread' : '';
    var html = '<li class="list-group-item d-flex justify-content-between align-items-center' + unreadCls + '" data-notification-id="' + item.id + '">';
    html += '<div class="avatar avatar-xs ' + bg + ' rounded-circle text-white"><i class="ti ' + item.icon + '"></i></div>';
    html += '<div class="ms-2 me-auto position-relative pe-4">';
    html += '<h6 class="mb-0">' + escapeHtml(item.title) + '</h6>';
    html += '<small class="text-body d-block">' + escapeHtml(item.message) + '</small>';
    if (item.actions && item.actions.length) {
      html += '<div class="d-flex flex-wrap gap-1 mt-2 notification-actions">';
    ...
    html += '<small class="text-muted position-absolute end-0 top-0 mt-1">' + escapeHtml(item.time) + '</small>';
    html += '</div></li>';
    return html;
  }

  function renderItem(item) {
    if (isNexlinkList()) return renderNexlinkItem(item);
  ...
  function markRead(id) {
    ...
    if (el) {
      el.classList.remove('dropdown-notifications-item-unread', 'nl-notify-unread');
