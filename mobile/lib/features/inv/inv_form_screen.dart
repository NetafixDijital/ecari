...
class InvFormScreen extends StatefulWidget {
  const InvFormScreen({super.key, this.invoiceType = 'SALES'});

  final String invoiceType;

  @override
  State<InvFormScreen> createState() => _InvFormScreenState();
}

class _InvFormScreenState extends State<InvFormScreen> {
  ...
  Widget build(BuildContext context) {
    if (_loading) {
      return Scaffold(
        appBar: AppBar(title: const Text('Yeni Satış Faturası')),
        body: const Center(child: CircularProgressIndicator()),
      );
    }
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.invoiceType == 'PURCHASE' ? 'Yeni Alış Faturası' : 'Yeni Satış Faturası'),
      ),
      body: Form(
        key: _formKey,
        child: ListView(
